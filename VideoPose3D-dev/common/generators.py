...
class ChunkedGenerator:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
                 chunk_length,rot,trans, pad=0, causal_shift=0,
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    if self.cameras is not None:
                        ...
                        self.rot[i] = self.rot[seq_i]
                        self.trans[i] = self.trans[seq_i]
                        if flip:
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    yield None, self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.rot[:len(chunks)], self.trans[:len(chunks)]
                elif self.poses_3d is None:
                    yield self.batch_cam[:len(chunks)], None, self.batch_2d[:len(chunks)], self.rot[:len(chunks)], self.trans[:len(chunks)]
                else:
                    yield self.batch_cam[:len(chunks)], self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.rot[:len(chunks)], self.trans[:len(chunks)]
...
class UnchunkedGenerator:
    ...
    def __init__(self, cameras, poses_3d, poses_2d,rot=None,trans=None, pad=0, causal_shift=0,
                 augment=False, kps_left=None, kps_right=None, joints_left=None, joints_right=None):
    ...
    def next_epoch(self):
        for seq_cam, seq_3d, seq_2d ,rot,trans in zip_longest(self.cameras, self.poses_3d, self.poses_2d,self.rot,self.trans):
            batch_cam = None if seq_cam is None else np.expand_dims(seq_cam, axis=0)
            batch_rot =  np.expand_dims(rot, axis=0)
            batch_trans =  np.expand_dims(trans, axis=0)
            ...
            yield batch_cam, batch_3d, batch_2d,batch_rot,batch_trans